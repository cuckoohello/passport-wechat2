    ...
    if (req.query && req.query.code) {
        ...
        self._oauth.getAccessToken(code, function (err, response) {
            ...
            if (~params.scope.indexOf('snsapi_base')) {
                ...
                self._oauth.getUser({ 'openid': params.openid, 'lang': 'zh_CN' }, function (err, profile) {
                    if (err) {
                    ...
                    try {
                        if (self._passReqToCallback) {
                            self._verify(req, params.openid, params, profile, verified);
                        } else {
                            self._verify(params.openid, params, profile, verified);
                        }
                    } catch (ex) {
                        return self.error(ex);
                    }
